			...
			columns: [
				{
					...
					field: 'patient',
					title: translate('locale.fields.patient'),
					sortable: false,
					width: 300,
					template: function (data) {
						var output = '<div class="d-flex align-items-center">\
            <div class="symbol symbol-40 symbol-sm flex-shrink-0">\
              <img class="" src="/' + data.patient.photo + '" alt="photo">\
            </div>\
            <div class="ml-4">\
              <div class="text-dark-75 font-weight-bolder font-size-lg mb-0">' + data.patient.name + '</div>\
              <a href="#" class="text-muted font-weight-bold text-hover-primary">' + data.patient.email + '</a>\
            </div>\
          </div>';
					...
					field: 'status',
					title: translate('locale.fields.status'),
					// callback function support for column rendering
					template: function (row) {						
						return '<span class="label label-lg font-weight-bold label-inline" style="color:rgb(255,255,255);background-color: ' + row.color + '">' + row.status + '</span>';
					},
				}, {
					field: 'init',
					title: translate('locale.fields.start_date'),
					template: function (row) {
						var output = '';

						output += '<div class="font-weight-bolder font-size-lg mb-0">' + row.init + '</div>';
					...
					template: function (row) {
						...
						output += '<div class="font-weight-bold text-muted">' + row.organization + '</div>';

						return output;
					}
				},{
					field: 'specialty',
					title: translate('locale.fields.area_job'),
					sortable: false,
					template: function (row) {
						var output = '';

						output += '<div class="font-weight-bold text-muted">' + row.specialty + '</div>';
