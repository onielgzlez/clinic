...
var KTAddUser = function () {
  ...
  var _initWizard = function () {
    ...
    /*let enabled = false;
        _formEl.querySelector('[name="password"]').addEventListener('input', function (e) {
          ...
          if (password === '' && enabled) {
            enabled = false;
            _validations[1].disableValidator('password').disableValidator('confirm-password');
          } else if (password != '' && !enabled) {
            enabled = true;
            _validations[1].enableValidator('password').enableValidator('confirm-password');
          }

          // Revalidate the confirmation password when the new password is changed
          if (password != '' && enabled) {
            _validations[1].revalidateField('confirm-password');
          }
        });*/
  ...
  var _initAvatar = function () {
    _avatar = new KTImageInput('kt_org_logo');
  }
